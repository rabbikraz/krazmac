...
export default function VideosGrid({ initialVideos, currentPage = 1, paramName = 'page', showFilters = true }: VideosGridProps) {
  ...
  useEffect(() => {
    if (page > 1 && filteredVideos.length > 0) {
      ...
      if (page > totalPages) {
        setPage(1)
        router.replace(`/videos?${paramName}=1`)
      }
    }
  }, [filteredVideos, page, router, paramName])
  ...
  return (
    <>
      {showFilters && <VideoFilters videos={initialVideos} onFilterChange={setFilteredVideos} />}
        ...
        <>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-8">
              <Link
                href={`/videos?${paramName}=${Math.max(1, page - 1)}`}
              ...
              <Link
                href={`/videos?${paramName}=${Math.min(totalPages, page + 1)}`}
